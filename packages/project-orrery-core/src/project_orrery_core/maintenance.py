...
_BINDING_FIELDS = {
    "workspace_id", "resolved_path", "head", "branch", "workstream_id", "session_phase",
...
def _maintenance_dir(project_root: Path, *parts: str, create: bool = False) -> Path:
    return _private_area(_repository_root(project_root), "maintenance", *parts, create=create)


def _manifest(project_root: Path) -> dict[str, Any]:
    path = _repository_root(project_root) / ".project-orrery.json"
    try:
...
def _queue_files(root: Path) -> list[Path]:
    directory = _maintenance_dir(root, "queue")
...
def run_maintenance_scan(
    ...
    *,
    ...
    try:
        ...
        counts = {
            "worktrees": len(inventory["entries"]),
...
def list_maintenance_queue(project_root: Path) -> dict[str, Any]:
    root = _repository_root(project_root)
    items = [
        validate_maintenance_contract(_read_regular_json(path, description="maintenance queue item"))
        for path in _queue_files(root)
    ]
    items.sort(key=lambda item: (str(item.get("lifecycle")), str(item.get("created_at")), str(item.get("item_id"))))
    return {"schema_version": 1, "items": items, "writes_performed": False, "network_performed": False}
...
def _fresh_cleanup(root: Path, item: Mapping[str, Any]) -> dict[str, Any]:
    policy = load_maintenance_policy(root)
...
def authorize_maintenance_item(
    ...
    *,
    ...
    fresh = _fresh_cleanup(root, item)
...
def execute_maintenance_authorization(
    ...
    *,
    ...
    registered, locked = _registered(root, selected)
    drift = (
    ...
    branch = fresh["workspace"]["git"].get("branch")
    head = fresh["workspace"]["git"].get("head")
    branch_retained = bool(branch and not _git(root, "show-ref", "--verify", str(branch), check=False).returncode)
    commit_retained = bool(head and not _git(root, "cat-file", "-e", f"{head}^{{commit}}", check=False).returncode)
    postflight = {
...
def maintenance_status(project_root: Path) -> dict[str, Any]:
    root = _repository_root(project_root)
    directory = _maintenance_dir(root)
    policy = load_maintenance_policy(root)
    ...
    if receipts_dir.is_dir():
        receipts = [
            validate_maintenance_contract(_read_regular_json(path, description="maintenance receipt"))
            for path in sorted(receipts_dir.glob("maintenance-receipt-*.json"))
        ]
    ...
    if authorizations_dir.is_dir():
        authorizations = [
            validate_maintenance_contract(_read_regular_json(path, description="maintenance authorization"))
            for path in sorted(authorizations_dir.glob("maintenance-authorization-*.json"))
        ]
    ...
    return {
        ...
        "status": "ready",
        "policy": policy,
        "host_preferences": load_host_preferences(root),
        "last_run": last,
        "queue": list_maintenance_queue(root)["items"],
