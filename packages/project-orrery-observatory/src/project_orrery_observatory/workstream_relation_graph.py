...
def _safe_source_link(value: Any) -> dict[str, Any]:
    ...
    if href is None:
        allowed_opaque = reference.startswith(OPAQUE_LINK_PREFIXES) or (
...
def _sanitize_node(value: Any, active_tip_ids: set[str]) -> dict[str, Any]:
    ...
    if node["origin"] == "legacy-session-projection" and (
        node["status"] in {"unknown", "stale"}
        or node["evidence_freshness"] in {"unknown", "stale"}
        or node["scope_status"] in {"unknown", "stale"}
    ):
        raise RelationGraphUnavailable("legacy-unknown", "Legacy relation evidence is Unknown or stale.")
    return node
...
def build_relation_graph_projection(provider_payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if provider_payload.get("relation_root_present") is not True:
        raise RelationGraphUnavailable("relation-store-absent", "Core relation store is absent; relation truth is Unavailable.")
...
def unavailable_relation_graph_projection(error: Exception) -> dict[str, Any]:
    ...
    messages = {
        "relation-store-absent": "Core relation store is absent. Relation facts remain Unavailable / Unknown.",
        "legacy-unknown": "Legacy relation evidence is Unknown or stale. No partial graph was generated.",
        "unsafe-source-link": "Core evidence link failed the safe-link whitelist. No partial graph was generated.",
        "dangling-node": "Core graph contains a dangling node reference. No partial graph was generated.",
        "dangling-evidence": "Core graph contains dangling evidence. No partial graph was generated.",
        "core-graph-invalid": "Core graph validation failed. No partial graph was generated.",
    }
    return {
        "projection_schema_version": PROJECTION_SCHEMA_VERSION,
        "contract_type": "workstream-relation-graph-observatory",
        ...
        "provider_id": PROVIDER_ID,
        "provider_schema_version": PROVIDER_SCHEMA_VERSION,
        "error": {"code": code, "message": messages.get(code, "Core relation provider is unavailable. No partial graph was generated.")},
...
function certainty(e){if(e.disposition==='suppress')return'suppressed';if(e.relation_type==='conflict-pair'&&e.certainty==='confirmed')return'confirmed-conflict';return e.certainty||'unknown'}function label(e){if(e.disposition==='suppress')return'SUPPRESSED';if(e.relation_type==='conflict-pair')return e.certainty==='confirmed'?'L3 / DIRECT':'COMPARE';if(e.certainty==='unknown')return'? UNKNOWN';if(e.lifecycle==='proposed')return'PROPOSED';if(e.lifecycle==='stale')return'STALE';return e.relation_type.replace('_',' ').toUpperCase()}
...
function select(kind,item){state.selection={kind,item};renderInspector();live.textContent=(kind==='node'?'Selected Workstream ':'Selected relation ')+(item.workstream_id||item.relation_id||item.id)}
function renderInspector(){inspector.replaceChildren();const selected=state.selection;if(!selected){inspector.append(el('p',null,data.status==='ready'?'Select a node or relation for exact Core evidence.':data.error.message));return}const item=selected.item;inspector.append(el('h3',null,item.workstream_id||item.relation_id||item.id));const axes=selected.kind==='node'?[['lifecycle',item.lifecycle_phase],['runtime',item.runtime_condition],['evidence',item.evidence_freshness],['scope',item.scope_status],['subsystem',item.primary_subsystem_id],['visibility',item.visibility],['observability',item.observability]]:[['relation',item.relation_type],['lifecycle',item.lifecycle],['certainty',item.certainty],['direction',item.source_workstream_id+' → '+item.target_workstream_id],['disposition',item.disposition||'edge']];axes.forEach(([k,v])=>{const row=el('div','wg-axis');row.append(el('span',null,k),el('b',null,v==null?'Unknown':String(v)));inspector.append(row)});const reasons=item.reason_codes||item.evidence_reason_codes||[];if(reasons.length){const ul=el('ul','wg-reasons');reasons.forEach(r=>ul.append(el('li',null,r)));inspector.append(ul)}const links=el('div','wg-evidence');(item.source_links||[]).forEach(link=>{const n=link.href?el('a',null,link.kind+' · '+link.ref):el('span',null,link.kind+' · '+link.ref);if(link.href){n.href=link.href;n.rel='noopener'}links.append(n)});if(!(item.source_links||[]).length)links.append(el('span',null,'No navigable source link · evidence remains '+(item.evidence_status||item.evidence_freshness||'Unknown')));inspector.append(links)}
function render(){const view=graph();svg.replaceChildren();const l=layout(view.nodes,view.edges);svg.setAttribute('viewBox',`0 0 ${l.width} ${l.height}`);const defs=se('defs');['cyan','amber','red','dim'].forEach((id,i)=>{const m=se('marker',{id:'wg-'+id,viewBox:'0 0 10 10',refX:9,refY:5,markerWidth:6,markerHeight:6,orient:'auto'});m.append(se('path',{d:'M0 0L10 5L0 10z',fill:['#5fcfc7','#efb759','#f07467','#8490a3'][i]}));defs.append(m)});svg.append(defs);view.edges.forEach(e=>{const a=l.positions.get(e.source_workstream_id),b=l.positions.get(e.target_workstream_id);if(!a||!b)return;const g=se('g'),p=edgePath(a,b),cls=certainty(e),path=se('path',{d:p.d,class:'wg-edge '+cls,'marker-end':`url(#wg-${cls==='confirmed-conflict'?'red':cls==='proposed'||cls==='unknown'?'amber':cls==='suppressed'||cls==='stale'?'dim':'cyan'})`}),hit=se('path',{d:p.d,class:'wg-edge-hit',tabindex:0,role:'button','aria-label':label(e)+' '+e.source_workstream_id+' to '+e.target_workstream_id});const text=se('text',{x:p.x,y:p.y,class:'wg-edge-label','text-anchor':'middle'});text.textContent=label(e);hit.addEventListener('click',()=>select('edge',e));hit.addEventListener('keydown',ev=>{if(ev.key==='Enter'||ev.key===' '){ev.preventDefault();select('edge',e)}});g.append(path,hit,text);svg.append(g)});view.nodes.forEach(n=>{const p=l.positions.get(n.workstream_id),status=n.is_active_tip?'active-tip':n.status==='blocked'||n.runtime_condition==='blocked-by-conflict'?'blocked':n.status;const g=se('g',{class:'wg-node '+status,tabindex:0,role:'button','aria-label':n.workstream_id+' '+n.runtime_condition});g.append(se('rect',{x:p.x,y:p.y,width:p.w,height:p.h,rx:8}));if(n.is_active_tip)g.append(se('rect',{x:p.x-5,y:p.y-5,width:p.w+10,height:p.h+10,rx:11,class:'wg-tip-ring'}));const title=se('text',{x:p.x+12,y:p.y+23,class:'wg-node-title'});title.textContent=n.is_cluster?'HISTORY · '+n.cluster_ids.length:n.workstream_id;const meta=se('text',{x:p.x+12,y:p.y+45,class:'wg-node-meta'});meta.textContent=n.primary_subsystem_id;const stateText=se('text',{x:p.x+12,y:p.y+68,class:'wg-node-state'});stateText.textContent=n.is_active_tip?'ACTIVE TIP':String(n.runtime_condition).toUpperCase();g.append(title,meta,stateText);g.addEventListener('click',()=>select('node',n));g.addEventListener('keydown',ev=>{if(ev.key==='Enter'||ev.key===' '){ev.preventDefault();select('node',n)}});svg.append(g)});renderLedger(view);page.querySelector('[data-wg-empty]').hidden=view.nodes.length>0;history.disabled=data.status!=='ready'||!(data.history_candidate_ids||[]).length;history.setAttribute('aria-pressed',String(state.history));history.textContent=state.history?'Collapse history':'Expand history';if(state.selection)renderInspector()}
...
function renderLedger(view){ledger.replaceChildren();if(!view.nodes.length&&!view.edges.length){ledger.append(el('div','wg-ledger-empty',data.status==='ready'?'No facts match these filters.':'Unavailable / Unknown · no partial graph generated.'));return}view.nodes.forEach(n=>{const b=el('button','wg-ledger-item '+(n.is_active_tip?'active-tip':n.status));b.type='button';b.append(el('b',null,n.is_cluster?'Collapsed history · '+n.cluster_ids.length:n.workstream_id),el('small',null,n.lifecycle_phase+' · '+n.runtime_condition+' · '+n.evidence_freshness));keyboardActivate(b,()=>select('node',n));ledger.append(b)});view.edges.forEach(e=>{const b=el('button','wg-ledger-item '+certainty(e));b.type='button';b.append(el('b',null,label(e)),el('small',null,e.source_workstream_id+' → '+e.target_workstream_id));keyboardActivate(b,()=>select('edge',e));ledger.append(b)})}
function options(select,values){[...new Set(values.filter(Boolean))].sort().forEach(v=>{const o=el('option',null,v);o.value=v;select.append(o)})}options(subsystem,(data.nodes||[]).flatMap(n=>[n.primary_subsystem_id,...(n.affected_subsystem_ids||[])]));options(runtime,(data.nodes||[]).map(n=>n.runtime_condition));page.querySelectorAll('[data-wg-lens]').forEach(b=>b.addEventListener('click',()=>{state.lens=b.dataset.wgLens;page.querySelectorAll('[data-wg-lens]').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));state.selection=null;render();renderInspector()}));subsystem.addEventListener('change',()=>{state.subsystem=subsystem.value;render()});runtime.addEventListener('change',()=>{state.runtime=runtime.value;render()});history.addEventListener('click',()=>{state.history=!state.history;render()});page.querySelector('[data-wg-reset]').addEventListener('click',()=>{state.history=false;state.subsystem='all';state.runtime='all';subsystem.value='all';runtime.value='all';render()});render();renderInspector();})();
"""
...
def render_workstream_relation_graph_panel(projection: Mapping[str, Any]) -> str:
    ...
    if error:
        failure = (
            '<div class="wg-failure" role="status"><b>Unavailable / Unknown</b>%s</div>'
            % html.escape(str(error.get("message", "Core relation provider is unavailable.")))
        )
    ...
    return (
        ...
        'data-title="Workstream Graph" data-authority="%s" data-read-only="true">'
        '<section class="wg-shell"><header class="wg-mast"><div><p class="wg-kicker">CORE RELATION GRAPH · READ ONLY</p>'
        '<h2>Workstream relation desk</h2><p>One Core fact set, three lenses. Unknown is preserved; no action executes here.</p></div>'
        '<div class="wg-seal %s">%s · SCHEMA 1</div></header>%s'
        '<section class="wg-controls" aria-label="Relation graph controls"><div class="wg-lenses" role="group" aria-label="Graph lens">'
        '<button class="wg-lens" type="button" data-wg-lens="succession" aria-pressed="true"><span>01</span><b>Succession</b></button>'
        '<button class="wg-lens" type="button" data-wg-lens="dependency" aria-pressed="false"><span>02</span><b>Dependency</b></button>'
        '<button class="wg-lens" type="button" data-wg-lens="conflict" aria-pressed="false"><span>03</span><b>Conflict</b></button></div>'
        '<div class="wg-filterbar"><label>Subsystem<select data-wg-subsystem><option value="all">All subsystems</option></select></label>'
        '<label>Runtime<select data-wg-runtime><option value="all">All runtime states</option></select></label>'
        '<button class="wg-button" type="button" data-wg-history aria-pressed="false">Expand history</button>'
        '<button class="wg-button" type="button" data-wg-reset>Reset view</button></div></section>'
        '<section class="wg-grid"><div class="wg-graph-panel"><div class="wg-panel-head"><div><p class="wg-panel-index">GRAPH / VERSIONED CORE PAYLOAD</p><h3>Relation topology</h3></div>'
        '<div class="wg-legend"><span><i></i>Confirmed</span><span><i class="broken"></i>Unknown / proposed</span><span><i class="conflict"></i>Confirmed conflict</span></div></div>'
        '<div class="wg-frame"><svg data-wg-svg role="img" aria-label="Interactive Workstream relation graph"></svg><div class="wg-empty" data-wg-empty hidden><b>No matching facts</b></div></div></div>'
        '<aside class="wg-inspector"><div class="wg-inspector-head"><span>SELECTION / CORE EVIDENCE</span><span>READ ONLY</span></div><div data-wg-inspector></div><div class="wg-readonly">No apply · undo · close · delete · merge · remote execution</div></aside></section>'
        '<section class="wg-ledger" aria-labelledby="wg-ledger-title"><h3 id="wg-ledger-title">Accessible relation ledger</h3><div class="wg-ledger-list" data-wg-ledger></div></section>'
        '<p class="wg-sr" aria-live="polite" data-wg-live></p><script type="application/json" data-wg-payload>%s</script>'
        '</section></article>'
        % (html.escape(authority, quote=True), status, html.escape(status.upper()), failure, payload)
    )
...
def inject_workstream_relation_graph(page: str, projection: Mapping[str, Any]) -> str:
    ...
    nav_candidates = [
        '<a class="nav-item" data-target="team-observatory"><span class="dot proposed"></span><span class="lbl">Team Observatory</span></a>',
        '<a class="nav-item" data-target="personal-observatory"><span class="dot state"></span><span class="lbl">Personal Observatory</span></a>',
    ...
    nav = '<a class="nav-item" data-target="workstream-relation-graph"><span class="dot proposed"></span><span class="lbl">Workstream Graph</span></a>'
