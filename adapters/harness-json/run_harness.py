...
def cli_arguments(request: Mapping[str, Any]) -> list[str]:
    ...
    relation_action = command.removeprefix("relations-") if command.startswith("relations-") else None
    arguments = ["relations", relation_action] if relation_action else [command]
    flag_order = {
