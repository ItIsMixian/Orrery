#!/usr/bin/env python3
"""Fail closed unless the Codex adapter's declared CLI dependency is usable."""
...
def load_requirement() -> dict[str, Any]:
    ...
    if not all(isinstance(value, str) and value for value in (distribution, entrypoint, minimum, maximum_exclusive)):
        raise ValueError(f"invalid adapter CLI requirement in {MANIFEST_PATH}: expected non-empty strings")
    parse_version(minimum)
    parse_version(maximum_exclusive)
    return requirement
