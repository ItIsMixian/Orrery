...
- [ ] [Orrery v0.3.0 Final RC and Promotion](implementation/plans/2026-08-30-v0-3-0-release-candidate-and-promotion.md)：Phase 0 COMPLETE；任务 merge `0f82d565...` 已非破坏性吸收中央 `dc5ff6e...`，产品路径等于 `ef145180...`。唯一 Candidate dry-run 被 reuse contract 拒绝且非 evidence-eligible；本线路已按 scope revision 3 停止，未运行 formal Candidate/build/runtime。push/main/tag/Release 仍分别授权。
...
1. 维护者／唯一协调者审阅 `0f82d565...` 的 Candidate dry-run contract refusal；当前 authority 不允许重试、拆批或替代 gate。
2. 只有新的 committed scope amendment 明确解决 Candidate contract 路径后才能继续；non-main Promotion、main、annotated tag 与 GitHub Release 均未授权。
