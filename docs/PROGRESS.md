...
| 多 Workstream 协作 | W1–W7 Canonical 基线与 ADR-0017 不变；ADR-0020 已接受 W program/W5-W7 phase hierarchy；W7.3 Core/capture 保留而 Graph 仍在改 | 原 W7.3 改为严格 module-scoped 输入、component/phase-local rank、独立区块二维排布与 block-pair 束线；保留 Core/capture，任何确认/apply/删除仍逐次本机授权 |
...
- [ ] [W7.3 Workstream Relation Capture](implementation/plans/2026-08-29-w7-3-workstream-relation-capture.md)：Core/capture 保留；新增 ADR-0020 hierarchy/bundle，并恢复组件内“已确认前序在左、后继在右”的偏序阅读。模块选择先严格裁剪 full-card 输入，跨模块只留 boundary stub；随后使用 component-local rank、二维 block packing 和 block-pair routing。真实截图接受前不得完成。
