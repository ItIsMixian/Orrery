...
- [ ] [Orrery v0.3.0 Final RC and Promotion](implementation/plans/2026-08-30-v0-3-0-release-candidate-and-promotion.md)：exact `e120aaa...` local gates 全绿并已推 Promotion ref；run `33449930707` 在任何 lane 前因 `--lane-list` stdout 被 docsite 日志污染而 preflight FAIL。scope revision 9 仅修 machine JSON 输出/AI-disabled discovery，形成新 SHA 后重走 exact gates；GitHub Release withheld。
...
1. 按 scope revision 9 修复 machine lane-list stdout，形成新 exact SHA并重新完成 Candidate/package/runtime/Promotion。
