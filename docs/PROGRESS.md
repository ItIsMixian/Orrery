...
Project Orrery v0.2.0 已公开发布。发布后的 B/H2 小样本已经完成：B 与 H2 在两个新高风险任务上均通过 2/2 独立验收，但 H2 总 input token 高 18.5%，没有通过采纳成本门。H2 不进入发布 Skill；读取证明和原始证据工具仅保留为研究 Harness。
...
- [ ] 将研究分支快进合并到本地 `main`，完成合并后的状态核验；本轮不发布新 Skill 版本。
...
将已验证的研究提交快进到本地 `main` 并核验工作树，然后把后续研究收缩到更简单的固定入口／任务定位与 Harness 专项验收，不修改发布 Skill。
