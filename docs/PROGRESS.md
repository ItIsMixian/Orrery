...
| 多 Workstream 协作 | W1–W7 Canonical 基线与 ADR-0017 不变；ADR-0020 已接受 W program/W5-W7 phase hierarchy；W7.3 Core/capture 保留而 Graph 仍在改 | 原 W7.3 改为严格 module-scoped 输入、跨组织 topology component、soft phase overlay、二维排布与局部短扇出；保留 Core/capture，任何确认/apply/删除仍逐次本机授权 |
...
- [ ] [W7.3 Workstream Relation Capture](implementation/plans/2026-08-29-w7-3-workstream-relation-capture.md)：Core/capture 保留；模块选择先严格裁剪 full-card 输入，关系组件跨 program/phase 优先共同排版，W 层级只作 local soft overlay；二维 packing 后仅允许端点附近短扇出，否决 W 单列高塔和跨页线路。真实截图接受前不得完成。
