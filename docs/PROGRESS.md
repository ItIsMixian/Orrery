...
- [ ] [Orrery v0.3.0 Final RC and Promotion](implementation/plans/2026-08-30-v0-3-0-release-candidate-and-promotion.md)：Phase 0 COMPLETE；scope revision 5 exact `ba230555...` 已完成 Candidate 81/81、双根 byte-identical package 与一次 external offline new-project PASS。剩余 final runtime matrix、Promotion、main、tag、Release 均未发生。
...
1. 由唯一协调者审阅 `ba230555...` 的 Candidate/package/offline evidence，并为剩余 final runtime matrix 或 non-main Promotion 单独提交后续 authority。
2. 当前不得自行运行 Promotion；main、annotated tag 与 GitHub Release 继续分别授权。
