...
- [ ] [Orrery v0.3.0 Final RC and Promotion](implementation/plans/2026-08-30-v0-3-0-release-candidate-and-promotion.md)：revision 5 已在真实页面恢复 lightweight Personal/Relation Inbox；同页又发现四个同端点 automatic Unknown lineage proposals。维护者授权 revision 6 用 append-only supersession 修到只留一个 current proposal，先看四条待确认页面、再 fresh route，之后才注册 Sol-medium Final RC。
