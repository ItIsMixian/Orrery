...
| 多 Workstream 协作 | 最终候选已包含 W1–W3、W4 health Personal Observatory、W5A Team foundation 与 W5B root-only Team Observatory；current blocker／reconciliation／workspace hygiene 已分层，Team UI 可完成本机全流程 | 完成联合 exact-SHA 验证后等待维护者确认；自动发现与真实多机/LAN 继续后置 |
...
- 多 Agent 候选已有 W1–W3 contract、修正健康语义的 W4 Personal 指挥台和 W5B Team 图形页；Team Mode 必须显式启用，中央只读/request-only，本机 receipt 始终 `execution_performed=false`。自动发现、真实多机、云 relay 与多设备迁移未实现。
...
- [ ] [多 Workstream 协作 Plan](implementation/plans/2026-08-19-multi-worktree-collaboration-protocol.md)：W4 health 与 W5B Team Observatory 已完成候选；完成联合门后等待维护者确认，不把 root-only/loopback UI 外推为真实多机 Team 支持。
...
2. **协作：** 完成 W4 health／W5B 最终候选 checks 后交由维护者体验 Personal/Team 图形流程；未经确认不推广 main，且不把 loopback 证据外推为真实 LAN。
