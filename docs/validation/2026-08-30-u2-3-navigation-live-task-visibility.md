...
Status: PASS — local A4/U2.3 integration Candidate; Promotion/public transition pending
...
- Central integration now contains A4 and U2.3 together; the routed Fast/Checkpoint and final 390px Browser replay are recorded below. W7.3, CI7 and REL3 remain independently owned.

## Central integration replay

- The integrator retained authority-first dispatch as ADR-0018 and normalized A4 portable operating rules to ADR-0019,
  updating every current link and the inventory source path. The inventory hash domain now canonicalizes CRLF to LF;
  digest `f786c4f2365e9b7b0e107d8365aba2ea5c81799b1b0a75a254a6aa06fe9eb7dd` passes on this Windows checkout while
  extra-byte tamper still fails closed.
- A4/Adapter/wheel focused suites passed 15/15; Unified/Personal passed 25/25 after integration fixes.
- Routed Fast passed 84/84 in 9.470426s under 15s; Checkpoint passed 89/89 in 17.311538s under 90s. The previous
  Candidate failures remain recorded above as branch history but are no longer current blockers.
- An independent local server on port 60414 was inspected at 390×844: help panel x=0, width/right=390, document
  horizontal overflow 0; the Ask Docs label is a non-interactive `P`, exactly one functional `qa-fab` remains, and
  console warning/error logs are empty. The temporary server was stopped; the existing central 60393 service was not
  stopped or modified.
- This completes local integration acceptance only. No exact non-main hosted Promotion, main/public/default switch,
  tag or Release is claimed.
