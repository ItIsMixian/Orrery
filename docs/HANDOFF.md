...
- 协作最终候选已包含 W1–W3 Personal Core、W4 root-only Personal Observatory 与 W5A opt-in Team foundation。W4 只读消费 W3；W5A 默认 Personal 零监听、Team enable 不监听、显式 serve 才启动，中央只读/request-only且不执行 shell／Agent／merge／delete。只有包含本记录的 exact SHA 位于 `main` 时 W4/W5A 才是 Canonical；当前仍等待维护者确认。
...
- 协作 Design 已推进到 W5A：Personal Core 和 Personal Observatory 已闭环，Team foundation 有 loopback/IP-literal、manual invite、Host确认、metadata sync、只读 projection 和 request receipt。自动发现、真实多机、Team UI、云 relay、多设备迁移与自动选主仍未实现。
