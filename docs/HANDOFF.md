...
- 协作 W1–W3 已进入包含本记录的 source：worktree/session、Scope/finding、acknowledgement、证据优先 review、推测性 integration、四种人工 decision、closure、bounded workspace inventory 与 advisory-only cleanup eligibility 均可用；实际 main 更新和四类清理动作仍需另行授权。该事实只有在包含本记录的 exact SHA 位于 `main` 时才是 Canonical；Observatory 与 Team runtime 仍未进入 Canonical。
...
- 协作 Design 已完成产品层收敛，W1–W3 已实现 Personal Core 的 identity/session/Scope/finding/review/integration/cleanup 基础；下一步整合 W4 Personal Observatory，W5 Team Mode 继续冻结。
...
11. 多人协作先读取 [ADR-0007](decisions/0007-multi-worktree-collaboration-and-branch-fact-scopes.md)、[活动 Plan](implementation/plans/2026-08-19-multi-worktree-collaboration-protocol.md)、[W2 Validation](validation/2026-08-22-w2-scope-finding.md)和[W3 集成 Validation](validation/2026-08-23-w3-canonical-integration.md)。W4 必须消费 W3 正式 contract，不得重写 review／cleanup 规则；根 `PROGRESS`／`HANDOFF` 由整合者同步。
