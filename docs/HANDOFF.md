...
- CI5 Promotion Throughput Optimization 从 Canonical `main@a4b0ed3` 建立。它保持 390 个当前 test ID、27 个逻辑 shard、双平台 exact-SHA、required checks 与安全预算，新增每 OS 十个物理 lane、独立 shard 子进程、lane receipt 和双 OS 并行。最终 Fast 57/57 为 3.235952s；Checkpoint 不调高 90s 预算，将 120.961576s 的真实 Git journey 只留在 Promotion 后，两次 81/81 为 42.806990s／43.071302s。本机 implementation／State／Validation 已收口；下一步仅冻结 clean exact SHA，取得 Fast 与新 25-job Promotion 双平台全绿并记录真实 overhead。
- R3 display-brand closeout、W7 collaboration source 与 CI4 opaque-token test reliability 已通过 CI4 exact `a4b0ed3` 的 Fast run `33233029172` 2/2、Promotion `33233029211` 59/59 和 required Windows／Ubuntu 双 PASS，并 fast-forward 至 `origin/main`。当前品牌为 Orrery；历史 v0.2.0、`project-orrery-*` 技术 ID 与发布资产保持不变。W7B 本机 execution 已实现但 self-host apply、默认 UI 执行入口和新 Release 仍未发生；关系图继续只读，中央 Team 无执行权。
...
- 协作最终候选已包含 W1–W3、W4 health Personal Observatory 与 W5A/W5B Team foundation/UI。W4 将 current blocker、stale reconciliation、legacy hygiene 分层；W5B 固定 loopback、root-only、同源/Host/cookie/body 门，中央 request-only且不执行 shell／Agent／merge／delete。只有 containing ref 为 main 时才是 Canonical；当前仍等待联合门与维护者确认。
...
- 协作 Design 已推进到 W5B：Personal/Team 本机图形流程可运行，Team foundation 有 manual invite、Host确认、metadata sync、只读 projection 和 request receipt。自动发现、真实多机、云 relay、多设备迁移与自动选主仍未实现。
