...
- W7.3 clean `44ea200` 已通过 focused closeout，中央 merge 为 `ae90974`；CI7 clean `111f4ab` 的中央合流为
  ...
  revision 9 以 one focused owner 和 Brand tiering 收口到目标 20/30。
...
15. **v0.3.0 Final RC：** `V0.3.0-central-integration-acceptance` 进入 scope revision 9；revision-7 页面已确认，
    现在只跑一次 exact focused owner、调整 Brand tier 并重新 real-window dry-run；允许后才 fresh route、同步
