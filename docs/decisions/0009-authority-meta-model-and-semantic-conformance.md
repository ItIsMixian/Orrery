# ADR-0009: Authority Meta Model、语义一致性与复杂性边界

Status: Accepted

Date: 2026-08-21

Origin: integrated from provisional decision `PO-DEC-AUTH-001`, approved by the maintainer on 2026-08-21

Amends: [ADR-0001](0001-project-orrery-self-hosting.md) Decision 1 and the self-hosting boundary

Clarifies: [ADR-0004](0004-platform-neutral-core-and-adapter-boundaries.md) conformance obligations without deciding the single implementation owner
...
> 本 ADR 在独立 integration worktree 中基于最新 integration history 获得正式编号。它只接受 AUTH-2、AUTH-3、AUTH-5、AUTH-6、AUTH-7、AUTH-8 的限定版本；AUTH-1“产品核心定位”和 AUTH-4“Core 是唯一 semantics owner”继续 pending。本 ADR 不授权立即代码重构。
...
AUTH-1 与 AUTH-4 继续 pending。

## Implementation and validation mapping

- Approved Design: [Authority Meta Model 语义设计](../design/authority-meta-model.md)
- Implementation Plan: deferred to the next planning conversation; no code extraction is authorized by this ADR alone
- State: [Authority Meta Model State](../state/authority-meta-model.md)
- Validation: [ADR-0009 文档与权威链集成](../validation/2026-08-21-authority-meta-model-adoption.md)
