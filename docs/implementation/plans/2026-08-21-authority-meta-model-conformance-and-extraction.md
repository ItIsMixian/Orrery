...
6. **兼容、自托管与发布**：ADR-0011 与 9-case legacy/supported/unsupported/invalid fixture 已冻结离散 support contract，Core 内部 capability judgment、CLI 只读报告、Candidate Observatory status signal 与 self-host 显式模型选择已经完成；没有导出稳定 Core API、修改 release 默认值或切换 managed Observatory。后续按显式 migration dry-run → project/release manifest projection → old-project/self-host/release 验证顺序推进。
