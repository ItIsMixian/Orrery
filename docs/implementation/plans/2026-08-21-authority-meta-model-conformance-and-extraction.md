...
6. **兼容、自托管与发布**：ADR-0011 与 9-case legacy/supported/unsupported/invalid fixture 已冻结离散 support contract，Core 内部 capability judgment、CLI 只读报告、Candidate Observatory status signal、self-host 显式模型选择和只读 migration dry-run 已经完成；没有导出稳定 Core API、实现 migration apply、修改 release 默认值或切换 managed Observatory。后续按显式 apply（含备份／恢复）→ project/release manifest projection → old-project/self-host/release 验证顺序推进。
...
- `packages/project-orrery-core/src/project_orrery_core/authority_migration.py`、`packages/project-orrery-cli/src/project_orrery_cli/authority_migrate.py`、`tests/test_authority_model_migration.py`：只读 migration planner 与 CLI dry-run；输入 bytes 的 hash、预测字段写入、备份范围、正交版本维度和失败关闭原因均可复核，当前命令拒绝任何非 dry-run 调用且没有 apply 路径。
