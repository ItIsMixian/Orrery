...
- 不修改 manifest、document schema、已发布 API、模板或发布契约；Candidate 可以增加未发布的内部 CLI
  检查点，但不得据此声称稳定公共接口已经建立。
...
[ADR-0011](../../decisions/0011-authority-model-version-and-compatibility.md) 已决定：项目 manifest 顶层使用正整数模型版本，release 声明默认值与离散支持集，缺失字段保持 `legacy-unversioned`，unsupported／unknown 只允许只读浏览并对 Authority 结论失败关闭，工具升级不得自动执行语义迁移。该决策解除只读 capability report 与显式 migration 设计的决策阻塞，但不自动授权发布或把 proposal 当成实现证据。
...
- `scripts/docsite/build_docsite.py` 与 `serve.py`：已有默认关闭的内部 sidecar 接线；只在维护者显式设置 `ORRERY_AUTHORITY_SHADOW_REPORT` 时运行，且不改变 HTML/stats。`docsite_qa.py` 与 `serve.py` 已把该 report 压缩成非权威 AI context，并给生成结果附加 receipt／提示；`docsite_insights.py`、页面 Authority projection 与 production switch 继续按独立检查点接入。
