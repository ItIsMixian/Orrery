...
- `scripts/docsite/build_docsite.py` 与 `serve.py`：已有默认关闭的内部 sidecar 接线；只在维护者显式设置 `ORRERY_AUTHORITY_SHADOW_REPORT` 时运行，且不改变 HTML/stats。`docsite_insights.py`、页面 projection、AI 约束与 production switch 继续按独立检查点接入。
