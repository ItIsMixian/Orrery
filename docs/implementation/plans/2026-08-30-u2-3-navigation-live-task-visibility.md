...
Status: Planned; authority scope frozen before resumed implementation
...
- [ ] App rail contains Project Overview, Docs & Search, Personal, Team, Workstreams, Maintenance and Routes & Trends.
- [ ] Remove the standalone Ask Docs navigation item; the floating Ask Docs control is the only Q&A entry.
- [ ] Move Routes & Trends from the project-document tree into the Orrery app rail.
- [ ] Remove standalone Authority/Facts & Rules navigation. A top-right help/system-status control opens a read-only
  surface containing Project Principles, Orrery Operating Rules and collapsed fact-interpretation status.
- [ ] Keep project documents in the author-document group only; preserve one sidebar and one scroll rail.
- [ ] Replace Unified `include_local_worktrees=False` root-only Personal snapshot with a lightweight active-task
  projection from Git worktree registry plus bounded Git-common-private session metadata.
- [ ] Use incremental Maintenance cache for workspace status; stale cache keeps task identity visible as “状态待刷新”.
- [ ] Do not open every worktree's source, full Scope, ignored files or diff during startup. Heavy evidence is loaded
  only on explicit detail/refresh.
- [ ] Dynamic status refresh notices new session revisions without restarting the whole service.
- [ ] Personal main view shows task code/name, phase, runtime, primary subsystem and evidence freshness while hiding
