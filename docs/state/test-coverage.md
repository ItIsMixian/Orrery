...
- 本机 cleanup 验证对 34 个移除目标逐项检查 registered path、允许根、tracked/untracked、ignored allowlist 与进程引用；第一批 31 个目标只含 1,377 个 `__pycache__` 和 5 个 `docs/_site`，约 117.1 MB。三个 stale session 在删除前复制到 Git-private archive 并逐项通过 SHA-256 一致性；最终 4 worktrees、0 reconciliation、2 intentionally retained hygiene。branch／commit 未删除。
...
- ADR-0007／ADR-0008 的当前 Worktree Candidate 覆盖 W1–W3 Personal contract、W4 health Personal 指挥台、W5A loopback Team Core／CLI 与 root-only W5C Team UI。仍无自动发现或真实多机 LAN 证据；本分支只有 Windows 本机 focused/checkpoint/browser 证据，promotion 仍要求中央 Candidate 与 containing docs SHA 的规定门。
