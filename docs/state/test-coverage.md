...
- W1／D1／C1 干净整合先暴露 C1 `application.facts.json` 在跨 worktree checkout 后的 CRLF hash mismatch；新增 fixture-source 专属 `eol=lf` 后保留原 manifest hash，联合专项 35/35 通过。最终默认全仓 273 项中 268 PASS + 5 existing skips，动态全仓 270 PASS + 3 Windows symlink privilege skips；Benchmark 24+6、frozen Pilot diff 0、integrated structure、隔离静态站、332 份 Markdown／843 个链接／0 unexpected missing、secret／禁用产物与 diff 检查通过，远端 CI 尚待本轮推送。
...
- ADR-0007／ADR-0008 的 Canonical Phase 1 已有 schema、Git fixture、status、Git-private session、create、guard、lifecycle、route 与条件 attach 测试。当前平台的 launch／rebind／message 均声明不支持，且 Adapter Skill 接线不等于宿主级写入拦截；Phase 2–4 的实际重叠／review／cleanup、Personal 指挥台与 Team Mode 网络测试仍未实现。
