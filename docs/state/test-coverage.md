...
- W2 Candidate 将 collaboration 专项扩展到 27/27：运行时 Git fixture 覆盖 committed／staged／unstaged／untracked／expected 五类来源、Direct／Authority／Semantic／Unknown、registry subsystem mapping、可配置凭据／release／schema 独占门、L1 自动 revision、L2 本机确认、L3 拒绝、route gate、Open／Acknowledged／Resolved／Stale、ack 失效和跨成员 `1/2 → 2/2`。实现提交 `de5152e` 后默认全仓为 278 项中 273 PASS + 5 existing skips，`ORRERY_TEST_BUILD=1` 动态全仓为 275 PASS + 3 Windows symlink privilege skips；最终结构、站点、链接与安全门见 W2 Validation。
...
- ADR-0007／ADR-0008 的 W2 Candidate 已在 Canonical Phase 1 的 schema、Git fixture、status、Git-private session、create、guard、lifecycle、route 与 attach 上补齐实际 scope／overlap／ack 测试。当前平台的 launch／rebind／message 均声明不支持，Adapter Skill 接线不等于宿主级写入拦截；review／integration／cleanup、Personal 指挥台与 Team Mode 网络测试仍未实现。
