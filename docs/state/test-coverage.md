...
- W7B 新增 `tests/test_workstream_relation_execution.py` 9 项：覆盖 execution schema/CLI/no-delete 轻量 contract，以及脱敏 W5C→W6→W5D→CI1/W5E 隔离 Git 全链的 legacy/explicit discovery、late CI、多 predecessor、no-lineage/Similarity Unknown、post-fork/sibling、四种 non-active predecessor、exact confirmation forgery/replay/expiry/cross-project/drift、multi-operation fault/recovery、completed takeover、deterministic receipt、undo/history 与 CLI 全闭环。最终 9/9 PASS，921.066s；W7A 15/15 PASS，19.791s；Fast 49/49 PASS，2.128s。CI1 inventory 为 366 unique IDs／26 shards／49 Fast，完整 W7B suite 恰好进入一个 Promotion shard。
- CI2 Worktree Candidate 保留上述 W7B claim 面，但把执行验证重排为一个 dependency-light Fast、一个 minimal-Git Checkpoint 和两个完整拓扑 Promotion journey；只有后两项建立完整 W5C→W6→W5D→CI1/W5E。`test-shards.json` 升为 schema 2，新增 90 秒 Checkpoint profile，并把 W7B 独立为 300 秒硬预算的 `team-relations-execution` shard。当前 inventory 为 362 unique IDs／27 shards／49 Fast／66 Checkpoint，0 missing／duplicate／dead selector；本机 Windows 最终 Fast 49/49 为 2.117s、Checkpoint 66/66 为 83.629s、W7B Promotion 4/4 为 264.738s，相对 921.066s 缩短约 71%。这些是 Worktree 测量，不是 hosted exact-SHA 证据。
...
- [2026-08-28 W7B Succession Apply／Undo／Legacy Inference](../validation/2026-08-28-w7b-succession-apply-undo-legacy-inference.md)
- [2026-08-28 CI2 Tiered Test Performance](../validation/2026-08-28-ci2-tiered-test-performance.md)
...
- ADR-0014/W7B 当前已有 Windows 本机 Fast/Checkpoint 与隔离 Git apply/undo/recovery 证据，但 self-host 真实项目只有 dry-run；没有真实 migration apply、W7C-B browser/UI、Ubuntu exact-SHA 或 Promotion 结果。W5D 双 clone runner 的既有 loopback 波动与 W7B relation transaction 相互独立，不能由本轮通过外推为 transport 稳定性修复。
