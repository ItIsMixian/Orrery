...
- W7C-A 新增 6 项 dependency-free 静态 prototype contract 测试，保护 fixture 的 provisional/version 边界、W5C→W6→W5D→CI1→W5E→W7C-A succession、同 base sibling、双前驱 dependency、Unknown／proposed 和带路径／验证面的 synthetic Direct conflict；同时保护三 lens、筛选、历史折叠、HTML fallback、键盘、390px media query、reduced-motion 与零生产／外部 runtime 依赖。真实 in-app Chromium 在 1280×720 点击 Dependency／Conflict、edge evidence、展开历史、双筛选与键盘 Enter，并在 390×844 点击移动 ledger dependency；两端 `scrollWidth` 分别 1265＜1280、375＜390，console 无 warning/error，asset origin 仅 loopback。
- W7C-B 新增 7 项生产 Observatory contract 测试，与 W7C-A 6 项合计 13/13 PASS；覆盖 corrected W7A real/synthetic payload、三 lens、多前驱／active tips／历史折叠、Unknown/stale/proposed/confirmed conflict、waiting/paused/blocked/failed、invalid/unsupported schema、missing relation root、dangling node/evidence、unsafe link、Core/provider failure、frontend no-Git/session semantics、zero-network/read-only 与 Personal/Team adjacency。CI1 inventory 为 370 unique IDs／26 shards／50 Fast，Fast 50/50；W7A/W7C-A/Personal/Team Checkpoint 45/45。真实 in-app Chromium 1280px 与 390×844 完成 click、Enter/Space、inspector、safe evidence link、filter、history、zero-overflow 与截图，accepted desktop run 的 console warning/error 为 0。
...
- [2026-08-28 W7C-A Workstream Graph visual prototype](../validation/2026-08-28-w7c-a-workstream-graph-visual-prototype.md)
- [2026-08-28 W7C-B Production Workstream Relation Graph Observatory](../validation/2026-08-28-w7c-b-production-workstream-relation-graph.md)
...
- ADR-0014/W7A/W7C-B 当前只有 Windows 本机 Fast/Checkpoint、隔离 Git fixture 与 in-app Chromium 证据；没有 W7B 真实 migration/apply/undo、Ubuntu exact-SHA 或 Promotion 结果。双 clone runner 在并发负载下的两次本机 loopback timeout 说明该邻接面仍有既有波动，独占 PASS 不外推为 transport 稳定性修复。
