...
- 当前这一“只恢复关系端点”的边界会让没有 relation 的已关闭任务从 Graph provider 消失；现有历史展开按钮无法恢复 provider 未提供的任务。Personal relation inbox 也仍以 revision、raw Workstream ID、英文 machine rationale 和 gate enum 为主要文案，维护者无法仅凭主卡理解接受后果。ADR-0026/W7.4 已接受纠正方向，但尚无实现。
...
- 没有独立于 worktree 的完整 closed-Workstream history index；清理后的零关系历史任务仍不能从当前 Graph 找回，关系确认卡也尚无 Core-owned human-readable consequence model。
