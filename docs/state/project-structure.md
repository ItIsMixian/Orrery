...
- 未发布的平台中立源码位于 `packages/project-orrery-{core,cli,observatory}/`。W7.2 Worktree Candidate 在 exact U2.1 continuation baseline 上声明 Core 0.1.17、CLI 0.1.21、Observatory 0.1.14；Core API 为 1，组件总状态为 `unreleased`。
...
- W7.2 Worktree Candidate 只重构 Observatory Graph presentation：从左到右的确定性 rank、固定可读节点、按 connected component 对齐的工程图路线、按链双向展开／收起、三 lens 真实端点、画布内 inspector 与移动 relation ledger。关系由实线／虚线／复合线和箭头表达，不在线路上覆盖文字；画布支持锚点式 `Ctrl + 滚轮` 缩放。Core relation schema／facts、W7.1 archive 证据与执行边界未改。
