...
- 未发布的平台中立源码位于 `packages/project-orrery-{core,cli,observatory}/`。A4/U2.3 local integrated Candidate 声明 Core 0.1.18、CLI 0.1.22、Observatory 0.1.18。Core API 为 1，组件总状态为 `unreleased`。
...
- ADR-0023 随后修正删除策略：手写 geometry 作为 frozen/manual/visibly-labelled legacy engine 保留，但 GX2
  页面获维护者接受前不做任何产品接线；ELK 失败不得自动回退。
