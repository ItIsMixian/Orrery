...
- 当前 source 将未发布 Core／CLI 推进到 0.1.7／0.1.12，并包含 W1–W3 的 Scope/finding/review/integration/closure/workspace cleanup contract；W4 Worktree Candidate 将 Observatory 推进到 0.1.1。Core API 仍为 1，公开 v0.2.0 与已发布资产不变。
...
- W4 Worktree Candidate 新增 root-only `build_personal_observatory.py` 与 Observatory 内部 Personal projection：它逐 worktree 调用 W1/W2 的 status／Scope／finding／lifecycle Core 合约，聚合为独立于总览仪表盘的只读四区页面，并允许把明确排除的并行分支只登记为 Unavailable 而不打开其工作目录或私有 session。当前 W3 分支使用该隔离槽；review queue、integration eligibility 与 cleanup eligibility 固定显示 `Unavailable / W3 not integrated`，不包含 W3 判定逻辑。
...
- W4 Worktree Candidate 已有 Personal Observatory 只读投影，但尚未 Canonical integration／发布；该 W4A checkpoint 仍以 W3 Unavailable 表达，后续 W4B commit 负责 W3 projection。Team 网络层仍未实现。
