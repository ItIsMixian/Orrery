...
- 未发布的平台中立源码位于 `packages/project-orrery-{core,cli,observatory}/`。A4 Candidate 声明 Core 0.1.18、CLI 0.1.22；U2.3 Worktree Candidate 将 Observatory 推进到 0.1.18。Core API 为 1，组件总状态为 `unreleased`。
- 薄平台层位于 `adapters/{codex,harness-json,claude-code,deepseek-harness}/`；Harness JSON A4 source 为 0.1.2，其余为 0.1.1，均 `experimental`／`unreleased`。Adapter 不拥有 canonical 作者模板、State、ADR、route semantics 或 Authority 规则。
...
- U2.3 Worktree Candidate 的 Personal active-task projection 只读取共享 Git worktree registry、Git-common-private bounded session JSON 和 Maintenance cache；公开 HTML/JSON 隐藏本机完整路径与原始 finding，缺失／损坏／cache stale 的已登记任务保持身份可见并标为待刷新。Git-private 数据仍不进入作者文档、发布包或远程同步。
