...
- [ADR-0015](../decisions/0015-orrery-brand-and-compatibility-contract.md) 已 Accepted，[Design](../design/orrery-rename-and-compatibility-contract.md) 已 Approved：[Plan](../implementation/plans/2026-08-28-orrery-rename-and-compatibility.md) 允许 R3 另立 Workstream 启动，R4/R5 仍为后续独立 Workstream。接受/批准不证明 R3 brand rollout、alias、package/CLI transition、本地 root rename、Saved Project 更新或 Codex 数据迁移已实现。本机 root/Saved Project 维护只有在 R3 exact-SHA 进入 main 后才可另行授权，且不依赖 R4/R5；Codex application-data D 盘迁移继续是再后续的独立任务。
