...
- Candidate Observatory 包已增加未导出的 parser shadow adapter：它接收真实 `build_docsite.parse_adrs` 输出，独立规范化 ADR lifecycle，在精确 numbered-ADR snapshot、`Unknown` scope 与 revision-content visibility 下调用注入的 Core evaluator；合成生命周期与当前仓库 parser 输出均匹配，伪造分歧按 `parser-gap` 分类。
- 当前 evaluator 是 experimental、fixture-bound 的 Candidate implementation：CLI 只完成 Accepted ADR 运行时 shadow，Observatory 只完成包级 ADR lifecycle shadow harness；没有稳定顶层 API、docsite build/serve 接线、consumer production switch、公开 schema/manifest 字段或发布实现，也不是 Canonical State 的实现声明。
...
- Observatory lifecycle shadow 尚未接入 `build_docsite.py`／`serve.py`；`predecessors`、`supersedes`、普通 ADR refs 与 State refs 仍明确属于 legacy graph/reference heuristics，尚无 Core relation conformance。
