...
- 当前 evaluator 是 experimental、fixture-bound 的 Candidate implementation：没有 Markdown/Git/Harness parser、稳定顶层 API、consumer migration、公开 schema/manifest 字段或发布实现，也不是 Canonical State 的实现声明。
...
- 尚无真实 CLI/Observatory parser 的双轨差异报告、consumer migration、公开语义版本字段、发布计划或 runtime Validation。
