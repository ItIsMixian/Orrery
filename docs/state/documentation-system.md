...
- W7.2 Worktree Candidate 将真实只读 Graph 改为单一从左到右 DAG：固定可读卡片、中文 rank lane、正交高对比边、每链独立历史折叠和默认关闭的技术详情抽屉。dependency／conflict 只从各自真实端点建图；空 dependency 不再显示孤立 active tips。桌面只显示主图并保留 1×1px 语义 ledger，390px 用同一事实／展开状态的普通任务关系列表替代微型图。
