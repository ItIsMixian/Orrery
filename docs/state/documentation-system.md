...
- ADR-0016 与 Approved Unified Observatory Design 已接受“一个用户入口／URL／导航壳、受管隐藏 helper”的目标。U2 Worktree Candidate 已实现 root-only/default-off 生产组合：复用现有 docsite 阅读、搜索、AI、作者信息架构和可识别视觉体验，通过一个 loopback URL 组合 Authority／Personal／Team／Graph／Maintenance；U1 synthetic prototype 仍不是 UI 规范，公开默认尚未切换。
...
- Unified Observatory 仍只是 U2 root-only/default-off Candidate；尚未进入默认 docsite、Skill template、managed-tool inventory、installer 或公开 Release。`start-docsite.bat`／`serve.py` 继续作为 legacy rollback 与当前公开兼容入口。
