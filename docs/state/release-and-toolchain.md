...
- ADR-0007／ADR-0008 的 Personal Phase 1 已进入 Canonical source：versioned contract、Git/config/subsystem/member/mode 解析、worktree create/status/guard/route、Git-private session、lifecycle transition、私有 attach 与 capability matrix 均可用，默认 Personal contract 保持 zero-network。当前平台均不声明 launch／rebind／message，且尚无宿主级任意写入拦截；Scope/finding、review／cleanup、`integrate`、Observatory 与 Team runtime 仍是后续目标，用户级 Skill／公开 v0.2.0 资产没有变化。
...
- 多 Workstream Phase 1 已进入 Canonical source；当前 Adapter 只有 Skill-level route／guard 接线与 caller-provided attach，没有宿主级写入拦截或已验证 launch／rebind／message。Scope/path collector、finding 计算、review／integration／cleanup、观测台投影、Team 网络面和本轮跨平台 CI 证据仍缺失。默认安装和公开 Skill 没有因 ADR-0008 开始监听网络。
- Claude Code 尚未完成成功认证与模型调用；DeepSeek Harness 的普通 wheel source-assets 阻塞已修复，且只有 manifest 中列出的 Adapter 0.1.0 精确 runtime 范围为 `verified`。两者均无公开分发或跨版本支持承诺，当前源码 Adapter 0.1.1／CLI 0.1.9 与其他 runtime／OS／模型不能继承旧验证结论。
