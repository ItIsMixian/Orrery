...
- 三个未发布组件初始版本均为 `0.1.0`；当前 W6 Worktree Candidate 为 Core 0.1.10／CLI 0.1.14／Observatory 0.1.6，Core API 仍为 1；仅 containing ref 为 main 时才是 Canonical。旧 Skill wrapper、managed-tool inventory 与冻结 v0.2 fallback 不变。
...
- 当前 W6 Worktree Candidate 包含 W1–W3 review/cleanup、W4 Personal 指挥台、W5A Team foundation、root-only W5C Team UI 与 W6 Phase 0–2 maintenance。仍缺 Phase 3 自动 removal、Phase 4 scheduler Adapter、自动发现、真实多机/LAN、自动选主、云 relay、多设备迁移与完整发行接线；Canonical 状态由 containing ref 决定，公开 v0.2.0 不变。
