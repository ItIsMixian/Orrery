...
- 三个未发布组件初始版本均为 `0.1.0`；Canonical source 已依次完成 W1/W2，当前 Core 0.1.5、CLI 0.1.10、Observatory 0.1.0。W4 Worktree Candidate 仅将 Observatory source 推进到 0.1.1，Core API 仍为 1。旧 `skills/project-orrery/scripts/` 路径是薄 wrapper，并保留可独立运行的冻结 v0.2 fallback；保留承诺覆盖 `0.3.x`，最早 `0.4.0` 才可移除。
...
- Phase 3 候选分支先把未发布 CLI 从 0.1.0 提升到 0.1.1：`scaffold`、`validate` 和 `check-update` 保留原人类输出，并新增 schema v1 opt-in JSON envelope。Authority migration 检查点把候选 CLI 依次推进到 0.1.2 dry-run、0.1.3 receipt-gated apply、0.1.4 scoped restore 和 0.1.5 future-release update compatibility；W1 Phase 0 又以 0.1.6 增加只读 `collaboration-contract`，W1.1 以 0.1.7 增加 `worktree status/session write`，W1.2 以 0.1.8 增加 `worktree create/guard`，W1.3 以 0.1.9 增加 lifecycle transition、Adapter route 与私有 attach，W2 以 0.1.10 增加 Scope/Finding，W3 Candidate 以 0.1.11 增加 `integrate`／`review`，再以 0.1.12 扩展 `review inventory/cleanup/cleanup-receipt` JSON 编排。既有 JSON envelope 与 update data schema 不变；blocked route／guard／review 使用稳定非零 exit。Codex Adapter 的已验证历史仍精确绑定 CLI 0.1.0，不能因当前源码版本变化而改写。
...
- ADR-0011 已定义 Authority Model 的公开正整数版本与离散支持集。self-host project manifest 已显式选择模型 1，neutral CLI 0.1.12 Candidate 保留 0.1.5 的兼容报告、receipt-gated migration／restore 和只读 update 判断；实际 release manifest、standalone installer、当前 v0.2 scaffold 和发布资产仍未投影默认值或支持集，普通工具升级也不会补写字段。
...
- ADR-0007／ADR-0008 的 W1/W2 已进入 Canonical source：versioned contract、Git/config/subsystem/member/mode 解析、worktree create/status/guard/route、Git-private session、lifecycle、私有 attach、Scope/finding 与 acknowledgement 均可用。W3 Candidate 复用这些 contract，提供 `integrate --dry-run`、证据优先 review package、四种人工 decision、integration eligibility、bounded workspace inventory、cleanup eligibility 与 Git-private closure/action receipt；默认 Personal Core 路径保持 zero-network，四类 cleanup 授权不互相隐含且所有实际 merge／push／删除都留给另行授权。当前平台均不声明 launch／rebind／message，且尚无宿主级任意写入拦截；Observatory 与 Team runtime 仍是后续目标，用户级 Skill／公开 v0.2.0 资产没有变化。
- W4 Worktree Candidate 的 `build_personal_observatory.py` 是与 M2.2 类似的 root-only opt-in entry；关闭开关时逐字节返回既有 base renderer，开启后再组合 W1/W2 Personal projection，并可与显式 Authority Candidate projection 组合。该脚本不进入 Observatory managed-tool 白名单或 Skill template，不改变默认 build／serve、wheel managed assets、installer、release manifest、用户级 Skill 或公开 v0.2.0。
...
- 多 Workstream W1–W3 source 已包含 review／integration dry-run／closure／bounded workspace inventory／cleanup eligibility；这些命令不执行真实 main 合流或清理，Canonical 与跨平台 promotion 状态由包含本段的 ref／exact-SHA checks 决定。当前 Adapter 仍只有 Skill-level route／guard 与 caller-provided attach；观测台投影和 Team 网络面仍缺失。
- W4 Personal 观测台只读投影目前仍是 Worktree Candidate；Team 网络面、Canonical integration 和本轮跨平台 CI 证据仍缺失。
- Claude Code 尚未完成成功认证与模型调用；DeepSeek Harness 只有 manifest 中列出的 Adapter 0.1.0 精确 runtime 范围为 `verified`。两者均无公开分发或跨版本支持承诺，当前源码 Adapter 0.1.1／CLI 0.1.12 与其他 runtime／OS／模型不能继承旧验证结论。
