...
- 多 Workstream W2 已进入 Canonical source：Scope/path collector、finding 计算、Scope Expansion B、lifecycle 与 route gate 可用；当前 Adapter 仍只有 Skill-level route／guard 与 caller-provided attach。review／integration／cleanup、观测台投影、Team 网络面和本轮跨平台 CI 证据仍缺失。
