...
- 三个未发布组件初始版本均为 `0.1.0`；本地 Canonical W1 Phase 0 将 Core 提升到 0.1.1、CLI 提升到 0.1.6。本分支 W1.1 Candidate 进一步提升到 Core 0.1.2／CLI 0.1.7，Observatory 保持 0.1.0，Core API 仍为 1。旧 `skills/project-orrery/scripts/` 路径是薄 wrapper，并保留可独立运行的冻结 v0.2 fallback；保留承诺覆盖 `0.3.x`，最早 `0.4.0` 才可移除。
...
- Phase 3 候选分支先把未发布 CLI 从 0.1.0 提升到 0.1.1：`scaffold`、`validate` 和 `check-update` 保留原人类输出，并新增 schema v1 opt-in JSON envelope。Authority migration 检查点把候选 CLI 依次推进到 0.1.2 dry-run、0.1.3 receipt-gated apply、0.1.4 scoped restore 和 0.1.5 future-release update compatibility；W1 Phase 0 又以 0.1.6 增加只读 `collaboration-contract` 人类／JSON 入口，W1.1 Candidate 以 0.1.7 增加 `worktree status` 与 `worktree session write`。既有 JSON 退出码与 update data schema 不变，Codex Adapter 的已验证历史仍精确绑定 CLI 0.1.0，不能因当前源码版本变化而改写。
...
- ADR-0011 已定义 Authority Model 的公开正整数版本与离散支持集。self-host project manifest 已显式选择模型 1，neutral CLI 0.1.7 Candidate 保留 0.1.5 的兼容报告、receipt-gated migration／restore 和只读 update 判断；实际 release manifest、standalone installer、当前 v0.2 scaffold 和发布资产仍未投影默认值或支持集，普通工具升级也不会补写字段。
...
- ADR-0007／ADR-0008 的 Phase 0 已进入本地 Canonical source，提供 versioned Core schema、dependency-free validation、只读 Git/config/subsystem/member/mode 解析和 `collaboration-contract` CLI；默认 Personal contract 明确所有网络能力关闭。W1.1 Candidate 增加稳定 JSON 的只读 `worktree status` 和显式 Git-private `worktree session write`，可识别 branch／HEAD／integration／dirty 漂移。`worktree create/overlap`、主目录守卫、完整 attach/rebind、review／cleanup、`integrate`、Observatory 与 Team runtime 仍只是后续目标，Skill／公开 v0.2.0 资产没有变化。
...
- 多 Workstream Phase 0 已进入本地 Canonical source，W1.1 Candidate 只增加私有 session 与只读 status；尚无写入守卫、worktree create、Scope/path collector、finding 计算、观测台投影、Team 网络面或 W1.1 CI 跨平台证据。默认安装和公开 Skill 没有因 ADR-0008 开始监听网络。
- Claude Code 尚未完成成功认证与模型调用；DeepSeek Harness 的普通 wheel source-assets 阻塞已修复，且只有 manifest 中列出的精确 runtime 范围为 `verified`。两者均无公开分发或跨版本支持承诺，当前 Candidate CLI 0.1.7 与其他 runtime／OS／模型不能继承 CLI 0.1.1 的验证结论。
