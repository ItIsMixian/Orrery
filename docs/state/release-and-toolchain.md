...
- ADR-0011 已定义 Authority Model 的公开正整数版本与离散支持集。self-host project manifest 已显式选择模型 1，neutral CLI 0.1.1 候选源码可只读报告兼容能力；component/release manifest、installer、新 scaffold 和 v0.2.0 发布资产仍未投影默认值或支持集，普通工具升级也不会补写字段。
...
- Authority Meta Model 已有 fixture-bound Core evaluator、内部兼容判断与 neutral CLI `validate` capability report，但没有稳定顶层 Core API、独立发行物、自动语义迁移、managed Observatory banner 或发布支持状态变化；v0.2.0 发布事实不变。
