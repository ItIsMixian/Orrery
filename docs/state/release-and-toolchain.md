...
- CI5 Worktree Candidate 只重排 self-host GitHub CI 的物理执行拓扑：27 个逻辑 shard、完整双平台 inventory、required smoke 名、exact-SHA／repository gate／aggregate 边界保持不变，物理 lane 将每 OS hosted shard jobs 从 27 收敛为 10，并解除 Ubuntu 对 Windows wave 的等待。当前未修改 branch protection、组件版本、发布清单、tag、Release 或 v0.2.0；只有 CI5 自身 exact-SHA 双平台验收后才可进入 Canonical source。
- CI4 exact `a4b0ed3` 已通过独立 Fast 2/2 与 Promotion 59/59 Windows／Ubuntu 门并 fast-forward 至 `origin/main`。其 leading-dash opaque token 可靠性修复不改变产品 parser、token 生成／storage、schema、transaction／receipt、品牌契约或发布事实。
...
- CI2 Worktree Candidate 将内部 test-shard manifest 升为 schema 2：保留 Fast 的非 Promotion 角色，增加带 90 秒硬预算的 Checkpoint，并允许独立 Promotion shard 声明预算。W7B 已从混合 `team-lan-core` 拆入 300 秒硬预算的 `team-relations-execution`；完整 Promotion inventory 仍要求每个最终 unittest ID 恰好一次，现为 362 IDs／27 shards。该变化没有修改 required-check 名称、Candidate-first exact-SHA 绑定、branch protection、tag、Release 或公开组件版本。
...
- W7A relation contract、W7B execution 与 W7C-B 图形页只存在于源码 Candidate；均未进入独立 wheel/release、Adapter capability manifest、默认 Observatory consumer 或 hosted Promotion。隔离本机 apply/undo 与本机 Chromium／邻接测试不能外推为 self-host 真实 migration、Ubuntu、跨设备、Canonical 或发布支持声明；W7C-B 继续没有图形执行入口。
