...
- 2026-08-22 经用户授权的 DeepSeek Harness rc.8 隔离运行已完成显式／隐式真实模型路由、CLI 缺失／不兼容失败关闭及 0→1→0 生命周期恢复；editable CLI 0.1.1 成功，普通 wheel CLI 因 Observatory source-assets 定位失败未通过完整兼容门，因此 `runtime_compatibility.verified` 仍为空。
...
- Claude Code 尚未完成成功认证与模型调用；DeepSeek Harness 已有真实显式／隐式模型调用与模型侧 CLI 失败关闭，但普通 wheel CLI 的 source-assets 定位失败阻止完整兼容门。两者均无公开分发或跨版本支持承诺，任何 runtime 声明只在对应 Validation 的精确范围内成立。
