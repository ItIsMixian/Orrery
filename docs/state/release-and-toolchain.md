...
- Claude Code 尚未完成成功认证与模型调用；DeepSeek Harness 的普通 wheel source-assets 阻塞已修复，且只有 manifest 中列出的精确 runtime 范围为 `verified`。两者均无公开分发或跨版本支持承诺，当前源码 CLI 0.1.6 与其他 runtime／OS／模型不能继承 CLI 0.1.1 的验证结论。
