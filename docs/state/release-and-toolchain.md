...
- 三个未发布组件初始版本均为 `0.1.0`；Canonical W3 source 当前为 Core 0.1.7、CLI 0.1.12、Observatory 0.1.0。W4 Worktree Candidate 将 Observatory source 推进到 0.1.2，Core API 仍为 1。旧 `skills/project-orrery/scripts/` 路径是薄 wrapper，并保留可独立运行的冻结 v0.2 fallback；保留承诺覆盖 `0.3.x`，最早 `0.4.0` 才可移除。
...
- 多 Workstream W1–W3 Canonical source 已包含 review／integration dry-run／closure／bounded workspace inventory／cleanup eligibility；这些命令不执行真实 main 合流或清理。当前 Adapter 仍只有 Skill-level route／guard 与 caller-provided attach；W4 Personal 观测台已只读消费 W3 数据，但目前仍只是 Worktree Candidate、尚未进入 Canonical，Team 网络面仍缺失。
