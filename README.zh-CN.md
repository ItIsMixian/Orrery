...
| DeepSeek Harness | 工作树包含未发布的 [profile Plugin Bundle Adapter](adapters/deepseek-harness/)，由插件注册 packaged Skill。 | `experimental`：`@deepseek-ai/dsh 0.1.0-rc.8` 已完成真实显式／隐式模型路由、CLI 缺失／不兼容失败关闭和可恢复卸载；但兼容 CLI 0.1.1 的普通 wheel 仍无法定位源码侧 Observatory assets，故 `verified` 保持为空。见 [runtime Validation](docs/validation/2026-08-22-deepseek-harness-adapter-stage-b-runtime.md)。 |
