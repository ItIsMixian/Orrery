...
| DeepSeek Harness | 工作树包含未发布的 [profile Plugin Bundle Adapter](adapters/deepseek-harness/)，由插件注册 packaged Skill。 | Adapter 发行状态仍为 `experimental` 且未发布；仅 Adapter 0.1.0 + `@deepseek-ai/dsh 0.1.0-rc.8` + Windows 11 build 26200 + Core 0.1.0 + CLI 0.1.1 + `deepseek-official`／`deepseek-v4-flash` 及已记录生命周期与失败关闭范围为 `verified`。见 [wheel runtime Validation](docs/validation/2026-08-22-cli-wheel-observatory-assets.md)。 |
