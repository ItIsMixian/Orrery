...
| DeepSeek Harness | 工作树包含未发布的 [profile Plugin Bundle Adapter](adapters/deepseek-harness/)，由插件注册 packaged Skill。 | `experimental`：`@deepseek-ai/dsh 0.1.0-rc.8` 已完成真实显式／隐式模型路由、CLI 缺失／不兼容失败关闭、可恢复卸载，以及修复后的普通 wheel CLI 路由；Candidate 仍待集成。见 [wheel runtime Validation](docs/validation/2026-08-22-cli-wheel-observatory-assets.md)。 |
