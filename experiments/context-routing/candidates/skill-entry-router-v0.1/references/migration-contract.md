...
Default scaffold creates missing files only. `--upgrade-tools` may replace only the installer whitelist and
must back up differing files first. Authored documents are migrated semantically after local instructions,
not by bulk replacement.
