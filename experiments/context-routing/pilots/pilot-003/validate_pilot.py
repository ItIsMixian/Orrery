#!/usr/bin/env python3
"""Validate a prepared or completed Project Orrery pilot-003 output root.
...
def validate_context_manifest(
    ...
    if variant == "A":
        if context is not None:
            errors.append(f"{label} must be null for variant A")
        return set()
    ...
    if variant == "B":
        ...
        if context["retrieval_strategy"] not in {"none", "single_file", "multi_file"}:
            errors.append(f"{label}.retrieval_strategy is invalid for variant C")
        if context["content_file_budget"] != 2:
            errors.append(f"{label}.content_file_budget must equal 2 for variant C")
...
def validate_receipt(
    ...
    if not isinstance(prewrite, dict) or set(prewrite) != {"context_manifest", "selected_evidence"}:
        ...
        if run["variant"] == "C":
            if not isinstance(selected, list) or not selected:
                errors.append(f"{run_key}: variant C requires non-empty selected_evidence")
            else:
        ...
        if event["event_type"] == "scope_expand":
            ...
            if run["variant"] in {"B", "C"}:
                ...
                if event["declared_before_access"] is not True:
                    errors.append(f"{label}: B/C read must be declared before access")
    ...
    if run["variant"] == "A" and content_reads[: len(FIXED_CHAIN)] != FIXED_CHAIN:
        errors.append(f"{run_key}: variant A receipt does not begin with the fixed seven-file content chain")
...
def validate_pilot(
    ...
    *,
    ...
    security_path = operator / "security-acceptance.py"
    ...
    if manifest.get("pilot_id") != "pilot-003" or manifest.get("prompt_revision") != "po-context-routing-pilot-003-v2":
    ...
    security_manifest = manifest.get("security_acceptance", {})
    if security_manifest.get("path") != "security-acceptance.py":
        errors.append("security acceptance path is missing from the manifest")
    if security_manifest.get("task_ids") != ["PO-CR-010", "PO-CR-011"]:
        errors.append("security acceptance task set differs from the apparatus contract")
    if sha256(security_path) != security_manifest.get("sha256"):
        errors.append("security-acceptance.py checksum does not match manifest")
    ...
    if (
        ...
        or not all(task_id in {"PO-CR-006", "PO-CR-010", "PO-CR-011"} for task_id in selected_task_ids)
    ):
    ...
    if (
        ...
        or not all(variant in {"A", "B", "C"} for variant in selected_variants)
    ):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.prepared_only:
        print(f"pilot-003 prepared apparatus OK: {completed or len(load_json(output_root / '_operator' / 'pilot-manifest.json')['runs'])} isolated runs")
    else:
        print(f"pilot-003 completed artifacts OK: {completed} comparable runs")
