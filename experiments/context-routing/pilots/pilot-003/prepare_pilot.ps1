...
$corpusPath = Join-Path $repoRoot "experiments\context-routing\corpus.json"
...
if ($config.pilot_id -ne "pilot-003" -or $config.external_context_policy -ne "repository_only") {
...
$securityAcceptanceSource = Join-Path $scriptRoot $config.security_acceptance.path
$securityAcceptancePath = Join-Path $operatorPath "security-acceptance.py"
[System.IO.File]::WriteAllText(
...
foreach ($selectedTaskId in $selectedTaskIds) {
    ...
    $matches = @($corpus.tasks | Where-Object { $_.id -eq $taskId })
    if ($matches.Count -ne 1) {
        throw "Task must resolve exactly once in corpus.json: $taskId"
    }
    $task = $matches[0]
    $baseCommit = [string]$task.source.base_commit
    ...
    if ($taskInstructions.Contains([string]$task.source.reference_commit) -or
        $taskInstructions.Contains($baseCommit)) {
        throw "Task packet leaks a historical commit identifier: $taskId"
    }

    $expectedWrites = @($task.oracle.reference_changed_paths)
    if ($expectedWrites.Count -lt 2) {
        throw "pilot-003 tasks must exercise more than one allowed product path: $taskId"
    }
...
$manifest = [ordered]@{
    ...
    security_acceptance = [ordered]@{
        path = "security-acceptance.py"
        sha256 = $securityAcceptanceSha256
        task_ids = @($config.security_acceptance.task_ids)
        evidence_origin = "operator"
    }
