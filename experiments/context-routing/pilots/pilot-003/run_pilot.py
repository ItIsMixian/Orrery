#!/usr/bin/env python3
"""Run Project Orrery pilot-003 with one reproducible local command.
...
def powershell_command() -> str:
    ...
    if command is None:
        raise RuntimeError("PowerShell is required by the pilot-003 operator scripts.")
    return command
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=[],
        choices=["A", "B", "C"],
        help="Routing variant to include; repeat for a subset. Defaults to A, B, and C.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    with RunnerLock(lock_path, resume=args.resume):
        ...
        if args.dry_run:
            ...
            print("pilot-003 dry run OK: preparation, hashes, repositories, and automation state are ready")
