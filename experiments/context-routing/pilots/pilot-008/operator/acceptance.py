#!/usr/bin/env python3
"""Independent behavior-first acceptance Oracle for Pilot 008."""
...
def check_progress(repository: Path, task_id: str, failures: list[str]) -> None:
    ...
    if task_id not in progress:
        failures.append(f"PROGRESS does not record {task_id}")


def check_030(repository: Path) -> tuple[list[str], list[str]]:
    failures: list[str] = []
    require_exact_changes(repository, "PO-CR-030", failures)
    run_probe(
        repository,
        textwrap.dedent(
            """
    ...
    check_progress(repository, "PO-CR-030", failures)
    return failures, []


def check_031(repository: Path) -> tuple[list[str], list[str]]:
    failures: list[str] = []
    require_exact_changes(repository, "PO-CR-031", failures)
    run_probe(
        repository,
        textwrap.dedent(
            """
            ...
            with tempfile.TemporaryDirectory() as temporary:
                ...
                try:
                    ...
                    indexes = {row[1] for row in connection.execute("PRAGMA index_list(feedback)")}
                    index_columns = [row[2] for row in connection.execute("PRAGMA index_info(idx_feedback_status_snoozed)")]
                ...
                assert "idx_feedback_status_snoozed" in indexes
                assert index_columns == ["status", "snoozed_until"]
    ...
    state = (repository / "docs/state/application.md").read_text(encoding="utf-8")
    for token in ("storage_schema_version: 2", "storage_v2_migration: implemented"):
        if token not in state:
            failures.append("Application State is missing " + token)
    check_progress(repository, "PO-CR-031", failures)
    return failures, []


def check_032(repository: Path) -> tuple[list[str], list[str]]:
    failures: list[str] = []
    require_exact_changes(repository, "PO-CR-032", failures)
...
def apply_positive_030(repository: Path) -> None:
    ...
    with (repository / "docs/PROGRESS.md").open("a", encoding="utf-8", newline="\n") as handle:
        handle.write("- PO-CR-030 completed with feedback regression coverage.\n")


def apply_positive_031(repository: Path) -> None:
    ...
    with (repository / "docs/PROGRESS.md").open("a", encoding="utf-8", newline="\n") as handle:
        handle.write("- PO-CR-031 completed with v1 preservation and idempotence coverage.\n")


def apply_positive_032(repository: Path) -> None:
    replace_tokens(
        repository / "docs/state/application.md",
        {
...
POSITIVE_APPLIERS = {
    "PO-CR-030": apply_positive_030,
    "PO-CR-031": apply_positive_031,
    "PO-CR-032": apply_positive_032,
}


def self_test() -> tuple[list[str], list[str]]:
    failures: list[str] = []
    with tempfile.TemporaryDirectory(prefix="orrery-p008-oracle-") as temporary:
        root = Path(temporary)
