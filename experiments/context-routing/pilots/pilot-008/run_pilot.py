#!/usr/bin/env python3
"""Prepare and preflight Pilot 008 P/S Scope Acquisition comparisons."""
...
spec = importlib.util.spec_from_file_location("orrery_pilot_005_runner_for_008", BASE_RUNNER_PATH)
if spec is None or spec.loader is None:
    raise RuntimeError("cannot load frozen Pilot 005 runner")
base = importlib.util.module_from_spec(spec)
spec.loader.exec_module(base)

runtime_spec = importlib.util.spec_from_file_location("orrery_app_server_runtime_for_008", SCOPE_SMOKE_PATH)
...
def initialize_fixture_repository(repository: Path) -> str:
    ...
    committed = _git(
        [
            ...
            "commit", "-m", "pilot-008 synthetic real-development baseline",
        ],
        repository,
...
def initialize_repository(repository: Path) -> None:
    ...
    if branch.returncode or branch.stdout.strip() != "pilot-008-fixture":
        raise RuntimeError("fixture repository is not on the isolated pilot-008-fixture branch")
    if not actual:
...
def skill_path(config: dict[str, Any], variant: str) -> Path:
    if variant not in config["variants"]:
        raise RuntimeError(f"unknown Pilot 008 variant: {variant}")
    ...
    if variant == "P":
        ...
        committed = _git(
            [
                ...
                "-c", "user.email=benchmark@local.invalid",
                "-c", "commit.gpgsign=false",
                "commit", "-m", "apply Pilot 008 task-first Agent entry",
            ],
            repository,
            environment=environment,
        )
...
def build_prompt(config: dict[str, Any], task: dict[str, Any], variant: str) -> str:
    ...
    return (
        prompt
        + "\n---\n\n# PROJECT ORRERY SKILL ENTRY UNDER TEST\n\n"
        + skill.strip()
        + "\n"
    )
...
def nested_preflight(config: dict[str, Any]) -> None:
    with tempfile.TemporaryDirectory(prefix="orrery-p008-nested-") as temporary:
        outer = Path(temporary) / "outer"
        outer.mkdir()
        initialized = _git(["init", "-b", "pilot-008-outer"], outer)
        if initialized.returncode:
            raise RuntimeError("cannot create outer preflight repository")
        for variant in config["variants"]:
            nested = outer / "runs" / f"PO-CR-030-{variant}" / "repository"
...
def preflight(config: dict[str, Any]) -> None:
    if config.get("pilot_id") != "pilot-008" or config.get("variants") != ["P", "S"]:
        raise RuntimeError("unexpected Pilot 008 configuration")
    if config.get("evidence_mode") != "codex-app-server-jsonrpc-scope-lock-v1":
        raise RuntimeError("Pilot 008 requires app-server Scope Lock evidence")
    ...
    if self_test.returncode:
        raise RuntimeError("Pilot 008 Oracle self-test failed: " + self_test.stdout + self_test.stderr)
    ...
    if scope_test.returncode:
        raise RuntimeError("Pilot 008 Scope analyzer self-test failed: " + scope_test.stdout + scope_test.stderr)
    ...
    if event_test.returncode:
        raise RuntimeError("Pilot 008 app-server validator self-test failed: " + event_test.stdout + event_test.stderr)
    protocol_self_test()
...
def runtime_handshake(codex: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="orrery-p008-appserver-handshake-") as temporary:
        root = Path(temporary)
            ...
            client.send(
                {
                    ...
                    "method": "initialize",
                    "params": {
                        "clientInfo": {"name": "project-orrery-pilot-008-preflight", "version": "0.1.0"},
        ...
        client.send(
            {
                ...
                "method": "initialize",
                "params": {
                    "clientInfo": {"name": "project-orrery-pilot-008", "version": "0.1.0"},
                    "capabilities": {"experimentalApi": True},
                },
            }
        )
...
def formal_pipeline_self_test(config: dict[str, Any]) -> None:
    with tempfile.TemporaryDirectory(prefix="orrery-p008-formal-pipeline-") as temporary:
...
def summarize(config: dict[str, Any], rows: list[dict[str, Any]], output_root: Path, hashes: dict[str, str]) -> None:
    ...
    lines = [
        "# Pilot 008 P/S Scope Acquisition raw comparison", "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not config.get("scope_usage_ordering_verified"):
        print(
            "Pilot 008 formal execution is disabled until the app-server usage/fileChange ordering smoke passes.",
            file=sys.stderr,
        )
        return 2
    try:
        ...
        print(f"Pilot 008 runtime preflight failed: {exc}", file=sys.stderr)
        return 2
    ...
    for task in tasks:
        ...
        if not all(row["apparatus_valid"] for row in pair_rows):
            ...
            print("Pilot 008 stopped after the first invalid apparatus pair; later samples were not started.", file=sys.stderr)
            return 3
