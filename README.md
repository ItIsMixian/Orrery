...
| DeepSeek Harness | The worktree contains an unreleased [profile Plugin Bundle Adapter](adapters/deepseek-harness/) that registers a packaged Skill. | Adapter distribution: `experimental` and unreleased. Runtime scope: `verified` only for Adapter 0.1.0 on `@deepseek-ai/dsh 0.1.0-rc.8`, Windows 11 build 26200, Core 0.1.0, CLI 0.1.1, `deepseek-official` / `deepseek-v4-flash`, and the recorded lifecycle and failure-closure scope. See [wheel runtime Validation](docs/validation/2026-08-22-cli-wheel-observatory-assets.md). |
