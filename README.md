...
| DeepSeek Harness | The worktree contains an unreleased [profile Plugin Bundle Adapter](adapters/deepseek-harness/) that registers a packaged Skill. | `experimental`: `@deepseek-ai/dsh 0.1.0-rc.8` completed real explicit/implicit model routing, CLI missing/incompatible failure closure, and recoverable uninstall. A compatible CLI 0.1.1 wheel still fails to locate source-owned Observatory assets, so `verified` remains empty. See [runtime Validation](docs/validation/2026-08-22-deepseek-harness-adapter-stage-b-runtime.md). |
