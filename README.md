...
| DeepSeek Harness | The worktree contains an unreleased [profile Plugin Bundle Adapter](adapters/deepseek-harness/) that registers a packaged Skill. | `experimental`: `@deepseek-ai/dsh 0.1.0-rc.8` completed real explicit/implicit model routing, CLI missing/incompatible failure closure, recoverable uninstall, and a post-fix normal-wheel CLI route. Candidate integration is still pending. See [wheel runtime Validation](docs/validation/2026-08-22-cli-wheel-observatory-assets.md). |
