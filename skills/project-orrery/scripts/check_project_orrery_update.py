#!/usr/bin/env python3
"""Check Project Orrery releases without changing the Skill or target project."""
...
LOCAL_MANIFEST_PATH = SKILL_ROOT / "release-manifest.json"
...
def fetch_latest(args: argparse.Namespace, local: dict[str, Any]) -> tuple[dict[str, Any] | None, str, str | None]:
...
def compatibility_with_environment(latest: dict[str, Any], target: dict[str, Any] | None) -> tuple[bool, list[str]]:
    compatibility = latest.get("compatibility", {})
    reasons: list[str] = []
    python_rule = compatibility.get("python", {})
    try:
        ...
        if sys.version_info[:2] < minimum_python:
            reasons.append(
                f"Python {sys.version_info.major}.{sys.version_info.minor} is older than required "
...
def evaluate(
    ...
    target_format = None
    if target:
        target_toolchain = target.get("toolchain_version") or target.get("version")
        target_schema = target.get("document_schema", 1 if target.get("name") == "project-orrery" else None)
        target_format = target.get("manifest_format", 1 if target.get("name") == "project-orrery" else None)
...
def main() -> int:
    args = parse_args()
    try:
        local = read_manifest(LOCAL_MANIFEST_PATH)
